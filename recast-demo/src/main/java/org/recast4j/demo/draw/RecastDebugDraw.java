...
public class RecastDebugDraw extends DebugDraw {
    ...
    protected void debugDrawCompactHeightfieldSolid(CompactHeightfield chf) {
        float cs = chf.cs;
        float ch = chf.ch;
    ...
    void debugDrawRegionConnections(ContourSet cset) {
        float alpha = 1f;
    ...
    private Contour findContourFromSet(ContourSet cset, int reg) {
        for (int i = 0; i < cset.conts.size(); ++i) {
            if (cset.conts.get(i).reg == reg) {
                return cset.conts.get(i);
            }
        }
        return null;
    }
    ...
    void debugDrawHeightfieldWalkable(Heightfield hf) {
        float[] orig = hf.bmin;
        float cs = hf.cs;
        float ch = hf.ch;
    ...
    void debugDrawCompactHeightfieldRegions(CompactHeightfield chf) {
        float cs = chf.cs;
        float ch = chf.ch;
    ...
    void debugDrawCompactHeightfieldDistance(CompactHeightfield chf) {
        if (chf.dist == null) {
            return;
        }
    ...
    void debugDrawPolyMeshDetail(PolyMeshDetail dmesh) {
    ...
    public void debugDrawNavMeshPoly(NavMesh mesh, long ref, int col) {
        if (ref == 0) {
            return;
        }
