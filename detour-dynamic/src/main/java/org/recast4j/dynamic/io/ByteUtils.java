...
class ByteUtils {
    ...
    static int getShortLE(byte[] data, int position) {
        return ((data[position + 1] & 0xff) << 8) | (data[position] & 0xff);
    }

    static int putInt(int value, byte[] data, int position, ByteOrder order) {
    ...
    static int putShort(int value, byte[] data, int position, ByteOrder order) {
        if (order == ByteOrder.BIG_ENDIAN) {
